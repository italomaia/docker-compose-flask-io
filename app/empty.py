...
class BlueprintSetupException(Exception):
    pass
...
class Empty(Flask):
    ...
    def add_blueprint(self, name, kw):
        for module in self.config['LOAD_MODULES_EXTENSIONS']:
            try:
                import_string('%s.%s' % (name, module))
            except (ImportError, AttributeError):
                msg = 'No {e_module} found in {e_name}.'
                print(msg.format(e_module=module, e_name=name))
    ...
    def configure_commands(self):
        """
        Add custom click commands here.
        """
        pass
